...
public sealed class MemoryByteWriterBigEndianTest(IEndianWriter writer)
{
    ...
    [TestCase(short.MinValue, new byte[] { MINUS_SIGN, 0 })]
    ...
    [TestCase(long.MinValue, new byte[] { MINUS_SIGN, 0, 0, 0, 0, 0, 0, 0 })]
    ...
    [TestCase(IntValues.C, new byte[] { 0, 0, 0x7F, 0xFF })]
    ...
    [TestCase(UShortValues.C, new byte[] { 0x7F, 0xFF })]
    public void Write_SingleValue_UShortEnum(UShortValues input, byte[] expected)
        => AssertEquivalent(w => w.WriteEnum(input), expected);

    [TestCase(0, 100, new byte[] { 0, 0, 0, 0, 0, 0, 0, 100 })]
    [TestCase(int.MaxValue, int.MinValue, new byte[] { 0x7F, 0xFF, 0xFF, 0xFF, MINUS_SIGN, 0, 0, 0 })]
