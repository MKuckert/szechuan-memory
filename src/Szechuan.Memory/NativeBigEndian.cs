...
internal sealed class NativeBigEndian : IEndianness
{
    public NativeBigEndian()
    {
        if (BitConverter.IsLittleEndian)
        {
            throw new InvalidOperationException($"Can't use {GetType().Name} on a system with little endian architecture. Use {nameof(ManagedBigEndian)} instead");
        }
    }
